...
- Task 07 已完成六个生产边界：Pydantic v2 HTTP 契约/隐私适配器、原子内存 Run Repository、带显式 migration 和 COMMIT-unknown 对账的 PostgreSQL Run/event Repository、FastAPI Run Gateway、支持严格断点续传/heartbeat/连接治理的有界 SSE 回放，以及 durable execution job/worker heartbeat/reclaim/API SIGTERM 恢复。
- Task 08 已完成生产向 Eval/Trace：OpenTelemetry 兼容 facade、W3C/MCP 传播、敏感属性 allowlist 脱敏、版本化数据集加载、RAG/Agent 确定性指标、故障注入与 baseline 回归报告、离线 CLI 与 tracing overhead 基准；单元测试仅使用内存导出器，不依赖 live Langfuse/Phoenix。
